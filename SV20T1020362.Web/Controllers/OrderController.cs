...
namespace SV20T1020362.Web.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult Cancel(int id = 0)
        {
            ...
            if (!result)
                TempData["Message"] = "Không thể thực hiện thao tác hủy đổi với đơn hàng này";
            return RedirectToAction("Details", new { id });
        }
        /// <summary>
        ...
        public IActionResult SearchProduct(ProductSearchInput input)
        {
            ...
            ApplicationContext.SetSessionData(PRODUCT_SEARCH,
        input);
        ...
        public IActionResult AddToCart(OrderDetail data)
        {
            ...
            if (existsProduct == null)   // nếu mặt hàng chưa có trong giỏ thì bổ sung thêm vào giỏ
            {
                shoppingCart.Add(data);
            }
            else // nếu mặt hàng có trong giỏ thì tăng số lượng và thay đổi giá bán
            {
        ...
        [HttpGet]
        public IActionResult AddressDetail(int id = 0)
        {
            ViewBag.OrderID = id;
            return View();
        }
        ...
        [HttpPost]
        public IActionResult AddressDetail(int id = 0, string deliveryAddress = "", string deliveryProvince = "")
        {
            if (string.IsNullOrWhiteSpace(deliveryAddress))
                return Json("Vui lòng nhập địa chỉ giao hàng");
            if (string.IsNullOrWhiteSpace(deliveryProvince))
                return Json("Vui lòng chọn tỉnh thành để giao hàng");

            bool result = OrderDataService.UpdateDeliveryOrder(id, deliveryAddress, deliveryProvince);
            if (!result)
                return Json("Không thể thay đổi địa chỉ cho đơn hàng này");
